...
def swap_dict_keys_to_values_naive(dictionary: _AnyDict) -> _AnyDict:
    ...
    for swapped_key in dictionary.values():  # type: Any
        if isinstance(swapped_key, (list, dict, bytearray)):
            raise TypeError(
                f"Dict key '{swapped_key}' should not be mutable object!"
            )
...
def swap_dict_keys_to_values_pythonic(dictionary: _AnyDict) -> _AnyDict:
    ...
    if any(
        filter(
            lambda swapped_key: isinstance(
                swapped_key, (list, dict, bytearray)
            ),
            dictionary.values(),
        )
    ):
...
if __name__ == "__main__":
    print(
        swap_dict_keys_to_values_naive({"a": 1, "b": 2})
    )  # -> {1: "a", 2: "b"}
    print(
        swap_dict_keys_to_values_naive({"a": 1, "b": 2, "v": [1, 2]})
    )  # -> raise TypeError

    print(
        swap_dict_keys_to_values_pythonic({"a": 1, "b": 2})
    )  # -> {1: "a", 2: "b"}
    print(
        swap_dict_keys_to_values_pythonic({"a": 1, "b": 2, "v": [1, 2]})
    )  # -> raise TypeError

    print(
        swap_dict_keys_to_values_object({"a": 1, "b": 2})
    )  # -> {1: "a", 2: "b"}
    print(
        swap_dict_keys_to_values_object({"a": 1, "b": 2, "v": [1, 2]})
    )  # -> raise TypeError
